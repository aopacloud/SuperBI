...
public class DashboardServiceImpl implements DashboardService {
    ...
    public List<DashboardDTO> search(DashboardQuery dashboardQuery) {
        ...
        condition.setSuperAdmin(sysUserService.isSuperAdmin(SecurityContextHolder.getUserName()));
...
//
//                }
                List<Long> shareIds = dashboardShareService.findDashboardIdsByUsername(SecurityContextHolder.getUserName());
            ...
            case ALL:
                ...
                if(!condition.isSuperAdmin()) {
                    List<Long> ids = dashboardShareService.findDashboardIdsByUsername(SecurityContextHolder.getUserName());
        ...
        dashboardDTOS.stream().forEach(dashboardDTO -> {
            ...
            dashboardDTO.setPermission(dashboardShareService.findMixedPermission(dashboardDTO.getId(), SecurityContextHolder.getUserName()));
        });
    ...
    public List<DashboardDTO> findByReport(Long reportId) {
        ...
        dashboardDTOS.stream().forEach(dashboardDTO -> dashboardDTO.setPermission(dashboardShareService.findMixedPermission(dashboardDTO.getId(), SecurityContextHolder.getUserName())));
        return dashboardDTOS;
    }
    ...
    public DashboardDTO findOne(Long id, Integer version) {
        ...
        dashboard.setPermission(dashboardShareService.findMixedPermission(dashboard.getId(), SecurityContextHolder.getUserName()));
    ...
    public DashboardDTO save(DashboardDTO dashboardDTO) {
        try {
            ...
            dashboardDTO.setCreator(SecurityContextHolder.getUserName());
            dashboardDTO.setOperator(SecurityContextHolder.getUserName());
