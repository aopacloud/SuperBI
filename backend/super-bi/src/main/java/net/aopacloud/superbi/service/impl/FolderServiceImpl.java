...
public class FolderServiceImpl implements FolderService {
    ...
    public FolderDTO save(FolderDTO folderDTO) {
        ...
        folder.setCreator(SecurityContextHolder.getUserName());
    ...
    public FolderResourceRelationshipDTO moveResource(FolderResourceRelationshipDTO relationship) {
        ...
        relationship.setCreator(SecurityContextHolder.getUserName());
        //1. 若 关系没变动 直接返回
    ...
    public Integer countResourceNum(FolderNode node, FolderQuery folderQuery) {
        ...
        if (node.getId() > 0) {
            return resourceCountService.countResourceByFolder(node, SecurityContextHolder.getUserName());
        }
